...
global coll_redshift_draw
global merg_redshift_draw


def Simulation(ps, pdx, draws, par_dict, num_param, redshifts=None, luminosities=None, durations=None, obs_pf=None, obs_t90=None, detector=None, options=None, vol_arr=None, kc=None, dl=None, plot_GRB=None, prior=False, sim=False, dsim=False, file=None):
    ...
    if pdx <= 3:
        ...
        # Update draw array
        draws[0] = redshift_sample_coll
        draws[1] = coll_kc
        draws[2] = coll_dl
        
    else:
        redshift_sample_coll = draws[0]
        N_coll = len(draws[0])
        coll_kc = draws[1]
        coll_dl = draws[2]
    
    if (isinstance(draws[3], int)==True) or (pdx > 3 and pdx <= 7):
        ...
        # Update draws array
        draws[3] = redshift_sample_merg
        draws[4] = merg_kc
        draws[5] = merg_dl
        
    else:
        redshift_sample_merg = draws[3]
        N_merg = len(draws[3])
        merg_kc = draws[4]
        merg_dl = draws[5]
    ...
    if (pdx <=3) or (pdx > 7 and pdx <= 9):
        ...
        # Update draws array
        draws[6] = lum_sample_coll
    else:
        lum_sample_coll = draws[6]
        
    if (isinstance(draws[7], int)==True) or (pdx > 3 and pdx <= 7) or (pdx > 7 and pdx <= 11):
        ...
        draws[7] = lum_sample_merg
    else:
        lum_sample_merg = draws[7]
